...
//            Log.d(TAG, "ping result content : " + stringBuffer.toString());
     ...
     * @return
     */
    public static String getSameNetWorkIp(Context context, String targetIp) {
        String sameNetWorkIp = null;
        if (targetIp != null && targetIp.contains(".")) {
            List<String> ipLists = NetWorkUtil.getEthernetIpAddress();
            Log.d(TAG, "getSameNetWorkIp: " + ipLists);
            String target = targetIp.substring(0, targetIp.lastIndexOf("."));
            for (String ip : ipLists) {
        ...
        Log.d(TAG, "getSameNetWorkIp: sameNetWorkIp=" + sameNetWorkIp);
        if (sameNetWorkIp == null) {
            return getIp(context);
        }
        return sameNetWorkIp;
    }
}
