...
public class ByteUtil {
    ...
    /**
     * 字节数组格式化，把字节数组每4个就加一段空格，每20个换行。<br>
     * 如果数据太多，格式化的时候比较耗时！！！ 手机的性能2万多字节可能会耗时20多秒。
     */
    ...
    public static int getIntByHigh(byte[] b, int offset) {
        int n = 0;
        int len = b.length;
        if (len >= offset + 4) {
            for (int i = 0; i < 4; i++) {
...
//     * @return
//     */
//    public static byte[] shortToByteArrayByLow(short value) {
//        byte[] shortBuf = new byte[2];
//        byte[] temp = new byte[2];
//        for (int i = 0; i < shortBuf.length; i++) {
//            int offset = i * 8;
//            temp[i] = (byte) ((value >>> offset) & 0xff);
//        }
//        shortBuf[0] = temp[0];
//        shortBuf[1] = temp[1];
//        return shortBuf;
//    }
     ...
     * int转byte[4]，低字节在前
     */
    public static byte[] intToArrayByHigh(int value) {
        byte[] bytes = new byte[4];
        for (int i = 0; i < bytes.length; i++) {
     ...
     * long转byte[8]，高字节在前
     */
    public static byte[] longToByteArrayByHigh(long value) {
        byte[] bytes = new byte[8];
        for (int i = 0; i < bytes.length; i++) {
            int offset = (bytes.length - 1 - i) * 8;
            bytes[i] = (byte) ((value >>> offset) & 0xff);
     ...
     * @return
     */
    public static long getLongByLow(byte[] b) {
        return getLongByLow(b, 0);
    }
