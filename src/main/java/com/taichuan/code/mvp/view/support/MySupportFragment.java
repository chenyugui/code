...
import com.taichuan.code.mvp.view.permission.PermissionBaseFragment;
 ...
 * fragmentation框架需要的SupportFragment
 */
public class MySupportFragment extends PermissionBaseFragment implements ISupportFragment {
    ...
    public <T extends MySupportFragment> T getParentDelegate() {
    ...
    /**
     * 如果Activity的Fragment栈的Fragment数量为1，则finish activity，否则pop fragment
     */
    protected void popOrfinishIfIsOnly() {
        MySupportActivity mySupportActivity = (MySupportActivity) getActivity();
        int count = mySupportActivity.getSupportFragmentManager().getBackStackEntryCount();
        if (count == 1) {
            getActivity().finish();
        } else {
            pop();
    ...
    public void popAllTopFragment() {
        MySupportFragment topFragment = (MySupportFragment) getTopFragment();
        if (topFragment != null && topFragment != this) {
            topFragment.pop();
            MySupportFragment preFragment = (MySupportFragment) topFragment.getPreFragment();
            while (preFragment != null && preFragment != this) {
                preFragment.pop();
                preFragment = (MySupportFragment) preFragment.getPreFragment();
            }
        }
    }
}
